...
from .views import ProfileDetailView, ProfileUpdateView, CustomPasswordChangeView, ProfileDeleteView
...
class ProfileUpdateViewTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(username='testuser', password='testPass123', email='old@example.com')
        self.client.login(username='testuser', password='testPass123')
        self.url = reverse('users:update_profile')

    def test_profile_update_valid(self):
        response = self.client.post(self.url, {'username': 'updateduser', 'email': 'new@example.com'}, follow=True)
    ...
    def test_profile_update_invalid(self):
        ...
        self.assertEqual(response.status_code, 200)
        messages = [m.message for m in get_messages(response.wsgi_request)]
        self.assertIn("There was an error updating your profile. Please check the form and try again.", messages)
...
class CustomPasswordChangeViewTest(TestCase):
    ...
    def test_password_change_invalid(self):
        ...
        self.assertEqual(response.status_code, 200)
        messages = [m.message for m in get_messages(response.wsgi_request)]
        self.assertIn("There was an error updating your password. Please check the form and try again.", messages)
...
class ProfileDeleteViewTest(TestCase):
    ...
    def test_profile_delete(self):
        ...
        messages = [m.message for m in get_messages(response.wsgi_request)]
        self.assertIn("Your profile has been deleted successfully. You can continue as a guest or register again!", messages)
...
class ProfileUpdateFormTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(username='testuser', password='testpassword', email='old@example.com')
