...
class ProjectCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, f"Project '{form.instance.name}' successfully created!"
        )
        return response
...
class ProjectDetailView(LoginRequiredMixin, UserPassesTestMixin, DetailView):
    ...
    def test_func(self):
        ...
        return (
            ...
            or project.owner == self.request.user
    ...
    def form_valid(self, form):
        ...
        if project.name != form.cleaned_data["name"]:
            ...
            if old_folder_path.exists():
                ...
                messages.success(
                    self.request, f"Project folder renamed to '{new_folder_path.name}'"
                )

        messages.success(
            self.request,
            f"Project details for '{form.instance.name}' successfully updated!",
        )
        return response
    ...
    def form_valid(self, form):
        ...
        if project_folder_path.exists():
            try:
                ...
                messages.success(
                    self.request,
                    f"Project '{project.name}' and its associated data have been successfully deleted!",
                )
            except Exception as e:
                messages.error(
                    self.request,
                    f"Project '{project.name}' was deleted, but failed to remove associated data: {e}",
                )
        else:
            messages.warning(
                self.request,
                f"Project '{project.name}' deleted, but no associated data was found.",
            )
...
class ProjectMemberRemoveView(LoginRequiredMixin, UserPassesTestMixin, DeleteView):
    ...
    def get_success_url(self):
        return reverse_lazy(
            "projects:detail_project", kwargs={"pk": self.object.project.id}
        )
    ...
    def handle_no_permission(self):
        messages.error(
            self.request, "You do not have permission to remove this member."
        )
