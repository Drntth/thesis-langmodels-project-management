...
class ProjectMemberModelTest(TestCase):
    ...
    def test_project_member_creation(self):
        ...
        self.assertEqual(
            str(self.project_member), "testuser - Developer in Test Project"
        )
...
class ProjectCreateViewTest(TestCase):
    ...
    def tearDown(self):
        project_folder_name = clean_filename(
            f"{self.user.username}_{self.project_data['name']}"
        )
        project_folder_path = (
    ...
    def test_project_creation(self):
        response = self.client.post(
            reverse("projects:create_project"), data=self.project_data, follow=True
        )
        self.assertEqual(response.status_code, 200)
        self.assertTrue(
            Project.objects.filter(name="Test Project", owner=self.user).exists()
        )
...
class ProjectUpdateViewTest(TestCase):
    ...
    def tearDown(self):
        ...
        project_folder_path1 = (
            Path(settings.MEDIA_ROOT) / "projects" / project_folder_name1
        )
        ...
        project_folder_path2 = (
            Path(settings.MEDIA_ROOT) / "projects" / project_folder_name2
        )
...
class ProjectMemberRemoveViewTest(TestCase):
    def setUp(self):
        ...
        self.url = reverse(
            "projects:remove_member", kwargs={"pk": self.project_member.pk}
        )
    ...
    def test_owner_can_remove_member(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertFalse(
            ProjectMember.objects.filter(pk=self.project_member.pk).exists()
        )
...
class ProjectCreationFormTest(TestCase):
    ...
    def test_deadline_in_past(self):
        form_data = {
            "name": "Test Project",
            "description": "A sample project.",
            "deadline": (datetime.date.today() - datetime.timedelta(days=1)).strftime(
                "%Y-%m-%d"
            ),
        }
        form = ProjectCreationForm(data=form_data, user=self.user)
...
class ProjectUpdateFormTest(TestCase):
    ...
    def test_valid_update_form(self):
        form_data = {
            ...
            "deadline": (datetime.date.today() + datetime.timedelta(days=5)).strftime(
                "%Y-%m-%d"
            ),
...
class AdminSiteTest(TestCase):
    ...
    def test_project_admin_list_display(self):
        ...
        self.assertEqual(
            ...
            ("name", "owner", "status", "deadline", "created_at"),
        )
    ...
    def test_project_member_admin_search_fields(self):
        admin_instance = ProjectMemberAdmin(ProjectMember, site)
        self.assertEqual(
            admin_instance.search_fields, ("user__username", "project__name")
        )
